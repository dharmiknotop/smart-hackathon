...
const AdminUserSection = () => {
  ...
  const getStoreDeatils = async () => {
    try {
      ...
      const { data } = await axios.get(
        "http://localhost:8081/api/user/getAllUsers"
      );
  ...
  const updateGarbageStatus = async (item) => {
    try {
      setLoading({
        loading: true,
        success: "",
      ...
      await axios.post("http://localhost:8081/api/user/updateStatus", {
        id: item._id,
        item,
      });
                ...
                {data?.map((item, index) => {
                  return (
                    <tr key={item?.storeName}>
                      ...
                      {console.log(item.isGarbageFull)}
                      <td className="w-10 p-3 text-sm text-white whitespace-nowrap">
                        <Select
                          color="teal"
                          size="md"
                          label="garbage collected"
                          value={item.isGarbageFull === true ? false : true}
                          onChange={(e) => {
                            console.log(item.isGarbageFull);
                            if (item.isGarbageFull === e) {
                              updateGarbageStatus(item);
                            }
                          }}
                          native
                        >
